<script lang="ts">
  ...
  function handleCellClick(cell: Cell) {
    ...
    if (editMode === 'wall') {
      cell.type = cell.type === 'wall' ? 'open' : 'wall'
      grid = grid  // trigger reactivity
    } else if (editMode === 'start') {
      // clear previous start
      for (const row of grid) {
        for (const c of row) {
          if (c.type === 'start') c.type = 'open'
        }
      }
      cell.type = 'start'
      grid = grid
  ...
  function run() {
    ...
    const result = algorithm === 'bfs'
      ? solveBFS(grid, se.start, se.end)
      : solveDFS(grid, se.start, se.end)
...
<SecretLayout>
  <div class="secret-root">
    ...
    <!-- controls bar -->
    <div class="controls">
      <AlgorithmPicker selected={algorithm} onSelect={(a) => { algorithm = a; reset() }} disabled={isRunning} />
